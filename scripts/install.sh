...
install_package() {
    ...
    if python3 -m litellm --version &>/dev/null; then
        ...
        if command -v pip3 &>/dev/null; then
            pip3 install 'litellm[proxy]' prisma --break-system-packages --quiet 2>/dev/null || true
        fi
        if command -v pip &>/dev/null; then
            pip install 'litellm[proxy]' prisma --break-system-packages --quiet 2>/dev/null || true
        fi
        if command -v pipx &>/dev/null; then
            pipx install 'litellm[proxy]' 2>/dev/null || true
            pipx install prisma 2>/dev/null || true
        fi
