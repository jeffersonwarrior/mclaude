...
# Mclaude Uninstallation Script
# One-line uninstaller: curl -sSL https://raw.githubusercontent.com/jeffersonwarrior/mclaude/main/scripts/uninstall.sh | bash
...
CONFIG_DIR="$HOME/.config/mclaude"
...
confirm_uninstall() {
    echo ""
    warn "This will uninstall mclaude and remove all configuration files."
    echo "The following will be removed:"
    echo "  - mclaude package (global and local installations)"
    echo "  - Configuration files in ~/.config/mclaude"
    echo "  - Cache files in ~/.local/share/mclaude"
    echo ""
...
uninstall_global() {
    log "Removing global mclaude package..."

    if npm list -g mclaude >/dev/null 2>&1; then
        if npm uninstall -g mclaude; then
...
# Remove npm link
remove_npm_link() {
    if command_exists npm && [ -L "$(which mclaude 2>/dev/null)" ]; then
        log "Removing npm link..."
        # Remove the symlink directly
        SYNCLAUDE_PATH=$(which mclaude)
...
# Remove symlink
remove_symlink() {
    if [ -L "$BIN_DIR/mclaude" ]; then
        log "Removing symlink..."
        rm -f "$BIN_DIR/mclaude"
...
verify_uninstall() {
    if command_exists mclaude; then
        error "mclaude command still found after uninstallation"
        warn "You may need to restart your terminal or manually clean up PATH"
        exit 1
    else
        success "mclaude has been successfully uninstalled"
    fi
}
...
main() {
    ...
    if ! command_exists mclaude && [ ! -d "$INSTALL_DIR" ] && [ ! -d "$CONFIG_DIR" ]; then
        warn "mclaude doesn't appear to be installed"
        exit 0
    fi
    ...
    log "Thank you for using mclaude!"
    echo ""
}
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                ...
                echo "1. Remove the mclaude package (both global and local)"
