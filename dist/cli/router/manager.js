"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RouterManager = void 0;
exports.getRouterManager = getRouterManager;
exports.resetRouterManager = resetRouterManager;
var litellm_proxy_1 = require("./litellm-proxy");
var RouterManager = /** @class */ (function () {
    function RouterManager(configManager) {
        this.proxy = null;
        this.configManager = configManager;
    }
    /**
     * Initialize and start the router if enabled
     */
    RouterManager.prototype.initializeRouter = function () {
        return __awaiter(this, arguments, void 0, function (options) {
            var config, status;
            var _a;
            if (options === void 0) { options = {}; }
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        config = this.configManager.config;
                        // Check if router is enabled in config
                        if (!((_a = config.liteLLM) === null || _a === void 0 ? void 0 : _a.enabled)) {
                            console.log("[Router] LiteLLM proxy disabled in config");
                            return [2 /*return*/, {
                                    running: false,
                                    url: "",
                                    uptime: 0,
                                    routes: 0,
                                }];
                        }
                        if (!this.proxy) {
                            this.proxy = new litellm_proxy_1.LiteLLMProxy(this.configManager);
                        }
                        console.log("[Router] Starting LiteLLM proxy...");
                        return [4 /*yield*/, this.proxy.start({
                                port: options.port || config.liteLLM.port || 8000,
                                host: options.host || config.liteLLM.host || "127.0.0.1",
                                timeout: options.timeout || config.liteLLM.timeout || 300000,
                                enabled: true,
                            })];
                    case 1:
                        status = _b.sent();
                        if (status.running) {
                            console.log("[Router] \u2705 LiteLLM proxy running at ".concat(status.url));
                            console.log("[Router] Routes configured: ".concat(status.routes));
                        }
                        return [2 /*return*/, status];
                }
            });
        });
    };
    /**
     * Stop the router
     */
    RouterManager.prototype.stopRouter = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!this.proxy) return [3 /*break*/, 2];
                        console.log("[Router] Stopping LiteLLM proxy...");
                        return [4 /*yield*/, this.proxy.stop()];
                    case 1:
                        _a.sent();
                        this.proxy = null;
                        console.log("[Router] ✅ LiteLLM proxy stopped");
                        _a.label = 2;
                    case 2: return [2 /*return*/];
                }
            });
        });
    };
    /**
     * Get router status
     */
    RouterManager.prototype.getRouterStatus = function () {
        var _a;
        return ((_a = this.proxy) === null || _a === void 0 ? void 0 : _a.getStatus()) || null;
    };
    /**
     * Check if router is running
     */
    RouterManager.prototype.isRouterRunning = function () {
        var _a;
        return !!(((_a = this.proxy) === null || _a === void 0 ? void 0 : _a.isEnabled()) && this.proxy.getStatus().running);
    };
    /**
     * Get proxy URL
     */
    RouterManager.prototype.getProxyUrl = function () {
        var status = this.getRouterStatus();
        return (status === null || status === void 0 ? void 0 : status.running) ? status.url : null;
    };
    /**
     * Cleanup on process exit
     */
    RouterManager.prototype.cleanup = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.stopRouter()];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    return RouterManager;
}());
exports.RouterManager = RouterManager;
// Singleton instance
var routerManagerInstance = null;
function getRouterManager(configManager) {
    if (!routerManagerInstance && configManager) {
        routerManagerInstance = new RouterManager(configManager);
    }
    return routerManagerInstance;
}
function resetRouterManager() {
    routerManagerInstance = null;
}
