...
class ClaudeLauncher {
    ...
    getProviderConfig(provider) {
        ...
        if (provider === "auto") {
            ...
            const enabledProviders = ["synthetic", "minimax"].filter((p) => this.configManager.isProviderEnabled(p));
            if (enabledProviders.length > 0) {
                return this.configManager.getProviderConfig(enabledProviders[0]);
            }
    ...
    getProviderApiKey(provider) {
        ...
        if (provider === "auto") {
            ...
            for (const p of enabledProviders) {
                const apiKey = this.configManager.getEffectiveApiKey(p);
                if (apiKey) {
                    return apiKey;
                }
            }
