...
export async function testConfigurationSystem(): Promise<void> {
  ...
  try {
    ...
    if (envVars.MINIMAX_API_KEY) {
      console.log(
        "MiniMax API key (first 20 chars):",
        envVars.MINIMAX_API_KEY.substring(0, 20) + "...",
      );
    } else {
      console.log("Trying direct process.env access...");
      console.log(
        "process.env.MINIMAX_API_KEY present:",
    ...
    const validation = envManager.validateEnvironmentVariables();
    ...
    console.log(
      "Synthetic API key present:",
      configManager.hasSyntheticApiKey(),
    );
    console.log("MiniMax API key present:", configManager.hasMinimaxApiKey());

    if (configManager.hasMinimaxApiKey()) {
      console.log(
        "MiniMax API key (first 10 chars):",
        configManager.getMinimaxApiKey().substring(0, 10) + "...",
      );
    }
    ...
    if (minimaxConfig) {
      console.log("MiniMax provider config:", {
        ...
        defaultModel:
          "defaultModel" in minimaxConfig ? minimaxConfig.defaultModel : "N/A",
    ...
    const validationResult = AppConfigSchema.safeParse(testConfig);
