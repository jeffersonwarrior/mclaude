...
describe("ConfigCliManager", () => {
  ...
  describe("showConfig", () => {
    it("should display the current configuration", async () => {
      ...
      expect(mockUI.table).toHaveBeenCalledWith(expect.objectContaining({
      ...
      expect(mockUI.info).toHaveBeenCalledWith(expect.stringContaining("Configuration is project-local"));
    });
  });
  ...
  describe("setConfig", () => {
    it("should set a selectedModel", async () => {
      ...
      expect(mockUI.success).toHaveBeenCalledWith("✓ Set selectedModel = new-model");
    });

    it("should set a selectedThinkingModel", async () => {
      await configCliManager.setConfig("selectedThinkingModel", "new-thinking-model");
      expect(mockConfigManager.setSavedThinkingModel).toHaveBeenCalledWith("new-thinking-model");
      expect(mockUI.success).toHaveBeenCalledWith("✓ Set selectedThinkingModel = new-thinking-model");
    });
    ...
    it("should set cacheDurationHours", async () => {
      ...
      expect(mockUI.success).toHaveBeenCalledWith("✓ Set cacheDurationHours = 48");
    });
    ...
    it("should set defaultProvider", async () => {
      ...
      expect(mockUI.success).toHaveBeenCalledWith("✓ Set defaultProvider = minimax");
    });

    it("should show error for invalid defaultProvider", async () => {
      await configCliManager.setConfig("defaultProvider", "invalid");
      expect(mockUI.error).toHaveBeenCalledWith("Invalid provider. Use: synthetic, minimax, or auto");
    });

    it("should show error for unimplemented keys", async () => {
      await configCliManager.setConfig("unimplementedKey", "value");
      expect(mockUI.error).toHaveBeenCalledWith("Invalid config key: unimplementedKey");
    });

    it("should show error for invalid keys", async () => {
      await configCliManager.setConfig("invalid.key", "value");
      expect(mockUI.error).toHaveBeenCalledWith("Invalid config key: invalid.key");
    });

    it("should set a provider config setting", async () => {
      await configCliManager.setConfig("synthetic.apiKey", "new-key");
      expect(mockConfigManager.setProviderConfig).toHaveBeenCalledWith("synthetic", "apiKey", "new-key");
      expect(mockUI.success).toHaveBeenCalledWith("✓ Set synthetic.apiKey = new-key");
    });
  ...
  describe("resetConfig", () => {
    it("should reset local configuration if confirmed", async () => {
      ...
      expect(mockUI.coloredSuccess).toHaveBeenCalledWith("✓ Local configuration reset to defaults");
    });
    ...
    it("should cancel local reset if not confirmed", async () => {
      ...
      expect(mockUI.info).toHaveBeenCalledWith("Local configuration reset cancelled");
    ...
    it("should show error for invalid scope", async () => {
      ...
      expect(mockUI.error).toHaveBeenCalledWith("Invalid scope: invalid. Use 'local' or 'global'.");
    });
    ...
    it("should advise manual reset for global scope", async () => {
      ...
      expect(mockUI.error).toHaveBeenCalledWith(
        expect.stringContaining("Global configuration reset is not yet fully implemented")
      );
    });
  });
  ...
  describe("showConfigContext", () => {
    it("should display information about local and global configs", async () => {
      ...
      expect(mockUI.info).toHaveBeenCalledWith(expect.stringContaining("Local config location"));
      expect(mockUI.info).toHaveBeenCalledWith(expect.stringContaining("Global config location"));
    });
  });
});